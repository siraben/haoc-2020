...
main = do
  ...
  let inp' = mkBlock <$> inp
  let allSides = snd =<< inp'
  let inp'' = [(id, (`uniqueSide` allSides) <$> s) | (id, s) <- inp']
  print (take 10 inp')
  print (take 10 allSides)
  let vv = [(x, countTrue id y) | (x, y) <- inp'']
  print (take 10 vv)
  print (filter ((== 4) . snd) vv)
  print (product (fst <$> filter ((== 4) . snd) vv))
-- print (part1 inp)
-- print (part2 inp)
-- defaultMain
--   [ bgroup
