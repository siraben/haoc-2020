...
-- mAdd x y = ((x + y) `mod` 9) + 1
cS c = (ord c - ord '0' - 1) `mod` 9
canon = map cS

dec n = ((n - 2) `mod` 9) + 1
nextCup :: Int -> [Int] -> Int
nextCup n l | dec n `elem` l = nextCup (dec n) l
            | otherwise = dec n
insertSub n s l = a ++ s ++ b
  where
    (a,b) = splitAt n l

removeSlice i n l = a ++ drop n b
  where
    (a,b) = splitAt i l
move (n,l@(h:t)) = (dest, h:(insertSub i pickup l'))
  where
    m = length l
    Just j = elemIndex n s
    s = cycle l
    pickup = take 3 (drop (j + 1) s)
    (pickup', l') = splitAt 3 t
    dest = nextCup n pickup
    Just i = succ <$> elemIndex dest l'

aa = foldl' (\c r -> 10 * c + r) 0
na = reverse . unfoldr (\x -> if x /= 0 then pure (x `mod` 10,x `div` 10) else Nothing)
...
main = do
  ...
  inp <- read . takeWhile isDigit <$> readFile dayFilename :: IO Int
  -- let inp' = canon inp
  let inp' = na inp
  print inp
  print inp'
